...
export default {
      ...
      assets: [
        {
          ...
          color: 'green',
          children: [
            { id: 111, name: 'aaa1', direction: 0 },
            { id: 112, name: 'aaa2', direction: 0 },
            { id: 113, name: 'aaa3' },
          ],
        },
        {
          ...
          type: 1,
          color: '#4BD314',
          children: [{ id: 221, name: 'bbb1' }, { id: 222, name: 'bbb2' }],
        },
        {
          id: 33,
          name: 'ccc',
        ...
        },
        {
          ...
          children: [
            ...
            { id: 442, name: '61.2.34.19', direction: 0, type: 1, time: '10:34:12' },
            { id: 443, name: '102.123.4.0', direction: 1, type: 1, time: '12:34:12' },
        ...
        },
        {
          ...
          children: [
            ...
            { id: 552, name: '128.189.1.143', direction: 1, type: 1, time: '16:14:13' },
            { id: 553, name: 'Accesssibility Features', direction: 0 },
            { id: 554, name: 'CMSTP', direction: 1 },
            { id: 555, name: 'BITS Jobs', direction: 0 },
            { id: 556, name: 'Exploitation for Credential Access', direction: 0 },
  ...
  methods: {
    createLeafBg(d, height, skewWidth, edge, delta) {
      ...
      if (d.angle < 90) {
        return `M${x + skewWidth},${y} L${x + width},${y} L${x + width},${y +
          height -
          skewWidth} L${x + width - skewWidth},${y + height} L${x},${y + height} L${x},${y +
          skewWidth} z`;
      } else if (d.angle < 180) {
        return `M${x},${y} L${x + width - skewWidth},${y} L${x + width},${y + skewWidth} L${x +
          width},${y + height} L${x + skewWidth},${y + height} L${x},${y + height - skewWidth} z`;
      } else if (d.angle < 270) {
        return `M${x + skewWidth},${y} L${x + width},${y} L${x + width},${y +
          height -
          skewWidth} L${x + width - skewWidth},${y + height} L${x},${y + height} L${x},${y +
    ...
    createLeafRegularBg(width, height, edge) {
      return `M0,0 L0,${edge} M${0},${height - edge} L0,${height} L${edge},${height} M${width -
    ...
    prepare() {
      ...
      // 创建defs里的内容
      this.root = d3.select('svg.scatter').attr('viewBox', this.chartHelper.getViewBox());
    ...
    createMarkers(container) {
      const types = [['red', '#FF4D4D'], ['blue', '#3BA1FF'], ['orange', '#F79A07']];
      container
            ...
            enter
              ...
              .attr('marker-end', d => {
                ...
                switch (d.source.data.direction) {
                  case 0:
                    return `url(#scatter-out-${data.type === 1 ? 'blue' : 'orange'})`;
                  case 1:
                    return `url(#scatter-in-${data.type === 1 ? 'blue' : 'orange'})`;
                  default:
                    return null;
                }
              })
              .attr('stroke', d => (d.source.data.type === 1 ? '#3BA1FF' : '#F79A07'))
              .call(enter =>
                enter
          ...
          enter => {
            const enterG = enter
              ...
              .attr(
                'transform',
                d =>
                  `translate(${(d.source.x + d.target.x) / 2},${(d.source.y + d.target.y) /
                    2}) rotate(${
    ...
    renderNodes(nodes) {
      ...
      const nodeList = this.nodeContainer
        .selectAll('g.node')
        .data(nodes.filter(item => item.level === 0), d => d.id)
        .join(
          enter => {
            const enterG = enter
              .append('g')
            ...
            enterG
              ...
              .attr('width', 30)
              .attr('height', 30)
              .attr('href', d => (d.data.type === 0 ? '#scatter-server' : '#scatter-end'))
              .attr('fill', d => (d.data.alarmLevel > 0 ? '#FF2424' : '#FF7615'));

            enterG.on('dblclick', this.expandDetailDialog).call(this.dragControl);
      ...
      this.nodeContainer
        .selectAll('g.child')
        .data(nodes.filter(item => item.level === 1), d => d.id)
        .join(
          enter => {
            ...
            smallG
              ...
              .attr('stroke', '#3BA1FF')
              .attr('fill', 'none')
              .attr('d', d => this.createLeafRegularBg(d.width + 6 * 2 + 26 + 4, 24, 6));
            smallG
              .append('use')
              .attr('x', 4)
              .attr('y', 4)
    ...
    dragUpdate(dragData) {
      ...
      this.linkContainer
        ...
        .attr(
          'transform',
          d =>
            `translate(${(d.source.x + d.target.x) / 2},${(d.source.y + d.target.y) / 2}) rotate(${
              d.source.angle > 180 ? d.source.angle - 270 : d.source.angle - 90
            }) translate(${-27},0)`
        );
    ...
    expandDetailDialog(d) {
      ...
      this.zoomControl.transform(this.root, d3.zoomIdentity.translate(x, y).scale(scale));
    },
    collapseDetailDialog() {
      this.detailStatus = false;
      this.selectId = null;
      this.zoomControl.transform(this.root, d3.zoomIdentity.translate(0, 0).scale(1));
    },
    handleDragOver(evt) {
      evt.preventDefault();
      return false;
    ...
    handleDrop(evt) {
      ...
      if (dataStr) {
        ...
        const x = (evt.clientX - (rect.left + transform.x - viewBox[0])) / transform.k;
        const y = (evt.clientY - (rect.top + transform.y - viewBox[1])) / transform.k;
